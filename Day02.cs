...
public class Day02
{
    ...
    void Part1_Input()
    {
        int result = SolvePart1(Input.Part1);
        Assert.Equal(1853, result);
    }

    private int SolvePart1(string input)
    {
        (int maxRed, int maxGreen, int maxBlue) = (12, 13, 14);

        Dictionary<int, List<(int red, int green, int blue)>> games = ParsePart1(input);

        var validGames = games
            .Where(x => x.Value
                .All(colors => colors.red <= maxRed && colors.green <= maxGreen && colors.blue <= maxBlue));
    ...
    private static Dictionary<int, List<(int red, int green, int blue)>> ParsePart1(string input)
    {
        ...
        Span<Range> cubesRanges = stackalloc Range[3];

        int count = inputSpan.Split(ranges, Environment.NewLine);
        for (var index = 0; index < count; index++)
        {
            ReadOnlySpan<char> line = inputSpan[ranges[index]];

            var game = int.Parse(line["Game ".Length..line.IndexOf(':')]);
            games[game] = [];

            var allSets = line[(line.IndexOf(':')+2)..];
            int setsCount = allSets.Split(setsRanges, ';');

            for (var j = 0; j < setsCount; j++)
            {
                ReadOnlySpan<char> set = allSets[setsRanges[j]];
                int cubesCount = set.Split(cubesRanges, ',');

                int red = 0;
                int green = 0;
                int blue = 0;

                for (int k = 0; k < cubesCount; k++)
                {
                    ReadOnlySpan<char> cube = set[cubesRanges[k]].Trim();

                    int spaceIndex = cube.IndexOf(' ');
                    int num = int.Parse(cube[..spaceIndex].Trim());

                    switch (cube[spaceIndex..].Trim())
                    {
                        case "red": red = num; break;
                        case "green": green = num; break;
                        case "blue": blue = num; break;
                        default: throw new UnreachableException("huh?");
                    }
                }

                games[game].Add((red, green, blue));
            }
    ...
    void Part2_Input()
    {
        var result = SolvePart2(Input.Part2);
        Assert.Equal(72706, result);
    }
    ...
    private int SolvePart2(string input)
    {
        ...
        var result = games
            .Select(x => (x.Value.MaxBy(r => r.red).red, x.Value.MaxBy(g => g.green).green, x.Value.MaxBy(b => b.blue).blue))
...
file static class Input
{
    ...
    public const string Part2 = Part1;
